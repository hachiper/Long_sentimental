...
class textCNN(nn.Module):
    ...
    def __init__(self, args,vectors=None):
        ...
        self.qurnn = QURNN(input_dim=args.embedding_dim, cell_dim=args.cell_dim, hidden_dim=args.hidden_dim,output_dim=self.label_num)
    ...
    def forward_test(self,x):
        ...
        c_n,_,out = self.qurnn(x)
        #print(c_n.shape)
        # dis = c_n[-1].squeeze()
        # dis = dis[-1,:]
        #print(dis.shape)
        # increasement = dis.unsqueeze(dim=1).repeat(1,300)
...
def textcnn_model_test(net, test_loader,args):
    ...
    with t.no_grad():
        for i, (data, label) in enumerate(test_loader):
            ...
            # np.savetxt('tensor.txt', data_np)
            # break
        percentage = 100 * correct / total
        t.save(net.state_dict(), "./model/"+str(percentage)+".pkl")
...
def parse():
    ...
    parser.add_argument('-sentence_max_size',type=int,default=1024,help='评论的最大长度')
    parser.add_argument('-cuda', type=bool, default=True)
    parser.add_argument('--lradj', type=str, default='type1', help='adjust learning rate')
    parser.add_argument('--cell_dim', type=int, default=300,help = 'dimension of cell')
    parser.add_argument('--hidden_dim', type=int, default=300, help='dimension of hidden')
...
if __name__ == "__main__":
    ...
    # test_set = test_set[:64*200]
    # test_label = test_label[:64*200]
